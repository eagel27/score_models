...
if TYPE_CHECKING:
    from score_models import EDMScoreModel, HessianDiagonal

from torch import Tensor
import torch

def denoising_loss(model: "EDMScoreModel", x: Tensor, *args: list[Tensor], **kwargs):
    B, *D = x.shape
    sde = model.sde
    ...
    D_theta = model.preconditioned_denoiser(t, xt, *args)
    # epsilon = model.reparametrized_score(t, xt, *args)
    # effective_score = (x - c_skip * (x + n)) / c_out
    return (c_out**2 * (epsilon - effective_score)**2).sum() / (2 * B)
